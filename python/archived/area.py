...
class CloudRFArea(CloudRFAPITemplated):
    """Area API class"""
    ...
    def download(self, select=None):
        ...
        for fmt in select:
            if fmt == 'url':
                ...
                self.download_from_archive(fmt)
...
class App:
    ...
    def parse_args(self, args):
        parser = argparse.ArgumentParser(description=textwrap.dedent(f'''
            ...
            This demonstration program utilizes the CloudRF Area API to generate any
            of the possible file outputs offered by the API from {CloudRFArea.file_types}.
        ...
        parser.add_argument('-s', dest='dl_types', nargs='+',
                            choices=CloudRFArea.file_types + ['all'],
        ...
        if 'all' in self.args.dl_types:
            self.args.dl_types = CloudRFArea.file_types
    ...
    def run_area(self):
        ...
        responses = []
        self.api = CloudRFArea(self.key, self.base_url, self.strict_ssl, self.save_response)
