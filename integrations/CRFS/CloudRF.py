...
class CloudRF:
    ...
    def __argparseInitialiser(self):
        ...
        self.__parser.add_argument('-w', '--wait', dest = 'wait', default = 0, help = 'Time in seconds to wait before running the next calculation.')
        self.__parser.add_argument('-rf', '--rfeye', dest = 'rfeye', default = False, help = 'IP address or domain name for a CRFS RFeye running NCP for live noise')
    ...
    def __calculate(self, jsonData):
        ...
        # BRING THE NOISE!
        if self.__arguments.rfeye:
            verbose = 0
            if self.__arguments.verbose:
                verbose = 1
            noise = asyncio.run(NoiseSource.rfeye(self.__arguments.rfeye,int(jsonData["transmitter"]["frq"]),int(jsonData["transmitter"]["bwi"]),verbose))
            print("Noise at %.1f MHz with %d MHz bandwidth at RFEye %s = %d dBm" % (jsonData["transmitter"]["frq"],jsonData["transmitter"]["bwi"],self.__arguments.rfeye,noise))
           
            # Assign noise if good, otherwise use template value
            if noise > -140:
                jsonData["output"]["nf"] = noise
                saveBasePath += "_" + str(noise)
