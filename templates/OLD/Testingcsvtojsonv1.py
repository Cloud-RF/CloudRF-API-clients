...
with open(sys.argv[1], newline='') as csvfile:
    ...
    for row in cr:
      ...
      template["template"]["owner"] = row["owner"]
      template["template"]["bom_value"] = int(row["bom_value"])
      ...
      template["transmitter"]["frq"] = int(row["frq"])
      template["transmitter"]["txw"] = int(row["txw"])
      ...
      with open(row["name"]+".json", "w",  encoding='utf-8') as jsonfile:
        json.dump(template, jsonfile,indent=3)
